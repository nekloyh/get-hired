"""Entry point for slices 0001–0002: evaluate a fixture answer with the Evaluator (one LLM call),
then fold that judgment into the Skill's Beta state (pure Python, no LLM)."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Evaluate one answer, then update the Skill state from the judgment."
    )
    parser.add_argument(
        "--answer",
