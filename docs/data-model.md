...
Verified against commit `3f19c99` on branch `audit/stabilize-2026-09-14`. Line refs are `path:line`; when the code moves,
the code wins. Terms: see `CONTEXT.md` (**Skill ledger** ≠ **usage ledger**, see Naming below).

## 1. Durable stores

| # | Store | Default path → env / `Settings` → container | Writers | Readers | Retention | Authoritative for |
|---|---|---|---|---|---|---|
| 1 | LangGraph checkpoints (SQLite) | `.session-checkpoints.sqlite` → `COACH_CHECKPOINT_DB` / `checkpoint_db` (`config.py:109`) → `/app/state/session-checkpoints.sqlite` (`Dockerfile:62`, `docker-compose.yml:14`). CLI `--checkpoint-db` default ignores the env var (`cli.py:907-911`). | Every graph step via `SqliteSaver`: CLI `cli.py:391` + stream loop `cli.py:273` (+ `update_state` on resume `cli.py:413`); web `web_api.py:890` + `_stream_graph` `web_api.py:1110` | CLI resume `cli.py:405` → `supervisor.py:199-208`; web resume `_checkpoint_values` `web_api.py:1047-1062` (feeds `max_questions` `:827-832`, `language_mode` `:1065-1078`) then `initial_state = None` `web_api.py:899` → `web_api.py:1110`; TTL sweep `prune_checkpoints` `web_api.py:467-508` | TTL sweep once per `create_app` (`web_api.py:703`, `:710-720`), i.e. on first `web_api.app` access (§2 import note): threads whose newest checkpoint is older than `checkpoint_ttl_seconds` (7 d default, `COACH_CHECKPOINT_TTL_SECONDS`, `0` disables — `config.py:123`). CLI never sweeps. | **The Session** — in-flight and completed state, resume, per-Session `skill_states`. |
| 2 | Skill ledger (JSON) | `.skill-ledger.json` → `COACH_LEDGER_DB` / `ledger_db` (`config.py:110`) → `/app/state/skill-ledger.json` (`Dockerfile:63`, `docker-compose.yml:15`). CLI `--ledger-db` defaults ignore the env var (`cli.py:918-922`, `:1049-1053`). | `save_posteriors` (`ledger.py:165-223`; lock `:56`, atomic rename `:216`) from web COMPLETE `web_api.py:943-948`, CLI `cli.py:522-524`, post-mortem `postmortem.py:186` | `load_priors` (`ledger.py:131-143`) from `web_api.py:913`, `cli.py:463`; `load_states` (`ledger.py:146-162`) from `postmortem.py:180` | None. One record per `candidate_id`, replaced on every save. | **Cross-session scoring memory** (decayed Beta priors, ADR 0006). Not any Session's result. |
| 3 | Usage ledger (JSONL) + `.unreconciled` sidecar | `<repo>/logs/usage-ledger.jsonl`, repo-anchored not CWD (`usage.py:63`) → `COACH_USAGE_LEDGER` read via `os.environ`, **not** a `Settings` field (`usage.py:77-78`) → `/app/state/usage-ledger.jsonl` (`Dockerfile:65`, `docker-compose.yml:22`). Sidecar `<ledger>.unreconciled` (`usage.py:84-90`). | `_append` (`usage.py:423-429`): token rows `llm.py:492-502`; `questions` via `reserve_questions` (`usage.py:900-905` — cap check + `record_questions` in one step under `_RESERVATION_LOCK` `:897`) from `web_api.py:839-841`; `session_run` via `begin_session_run` (`usage.py:779-782`) from `web_api.py:907`, `cli.py:462`; `quota_exhausted` `llm.py:448`; `quota_retry` via `clear_run_rails_for_resume` (`usage.py:1030`) from `web_api.py:853`, `cli.py:425`. Sidecar: `_flush_faults` `usage.py:371-392`, rewritten whole on reconcile `usage.py:603-625`. | `_all_rows` (`usage.py:635-655`); rails `usage.py:823, 846, 871, 900, 970`; per-call gate `llm.py:427` → `accounting_gate` `usage.py:532-551`; `coach usage` `cli.py:707-760`; bench preflight `cli.py:658-668`; forge `cli.py:772-776`; `--reconcile` `cli.py:710-715` → `usage.py:554` | None. Append-only, never rotated. | **Spend** (day / run / identity) and the `insufficient_quota` latch — the only record; the provider API exposes no daily quota (`usage.py:3-5`). |
| 4 | Exports (Markdown) | `data/exports/<safe-id>.md` → `COACH_EXPORTS_DIR` / `exports_dir` (`config.py:111`, `web_api.py:142`) → `/app/state/exports` (`Dockerfile:64`, `docker-compose.yml:16`). `<safe-id>` = the Session id if it matches `SAFE_SESSION_ID` (`web_api.py:146`), else its sha256 (`export_path`, `web_api.py:332-343`). CLI `--export-markdown <any path>` (`cli.py:1012`). | `_persist_export` on COMPLETE (`web_api.py:942`, `:1081-1091`) → `exporter.py:14-19`; CLI `cli.py:526-527` | Export endpoint, only after a RAM miss (`web_api.py:696-701`) | None. | Nothing — a rendering of #1. After a restart, or once the id is evicted from RAM (§2), it is the only copy the export endpoint can serve (§5 item 1). |
| 5 | Chroma persist dir (concept notes) | `.chroma/` → `COACH_CONCEPT_PERSIST_DIR` / `concept_persist_dir` (`config.py:119`); kind `concept_store="auto"` (`config.py:118`). Per-embedder subdir `<dir>/<model, "/"→"__">` (`concepts.py:309-320`): `BAAI__bge-small-en-v1.5`, `intfloat__multilingual-e5-small` (`concepts.py:23, 28`). No container override; the image installs without `--extra rag` (`Dockerfile:42`), so `auto` resolves to `memory` there (`concepts.py:323-331`). | `ChromaConceptStore.ingest` (`concepts.py:219`) via `build_concept_store(seed=True)` at every Session start: web `web_api.py:867-872` (demo mode forces `memory`, `:868`), CLI `_cmd_session` `cli.py:350-361`; `coach ingest-concepts` (`cli.py:819-821`). Chroma writes are local-only — in the container every one of these calls lands in the memory store. | `lookup` (`concepts.py:233-261`) via `lookup_concept` (`concepts.py:374-383`) — the Interviewer's tool | None. Derived; rebuilt by re-ingest. | Nothing — an index over the YAML shelf. |
| 6 | Chroma resource store (Study Planner) | No `Settings` field; CLI-only: `--resource-store` (default `memory`) / `--resource-persist-dir` (default `.chroma`, `cli.py:996-1005`, `:1073-1081`) → `build_resource_store` (`resources.py:391-406`) → `ChromaResourceStore.create` → `chromadb.PersistentClient(path=persist_dir)` (`resources.py:140-171`), collection `RESOURCE_COLLECTION = "resources"` (`resources.py:22`). Web hardcodes `memory` (`web_api.py:873`). **Caveat for #5:** the default root is the same `.chroma` and this store is **not** namespaced per embedder, so `--resource-store=chroma` writes a second collection into the concept store's root directory. | `ingest` (`resources.py:190`) via `build_resource_store(seed=True)` `cli.py:364-368`, `:565-568`; `coach ingest-resources` (`cli.py:828-831`, `--persist-dir` default `.chroma` `:1174`) | `search_resources` (`resources.py:409`) from `study_planner.py:244` | None. Derived. | Nothing — an index over `SEED_RESOURCES`. |
| 7 | Server log file | None by default (stderr only) → `COACH_LOG_FILE`, env-only, **not** a `Settings` field: read once at import `web_api.py:745` → `configure_session_logging` (`web_api.py:427-464`; `RotatingFileHandler(maxBytes=10 MiB, backupCount=5)` `:457`). Compose sets `/app/state/logs/coach-api.log` (`docker-compose.yml:26`); the Dockerfile does not. CLI `coach api --log-file` writes the env var before the import (`cli.py:846-847`; flag `:1181-1188`). | Every `interview_coach` logger record, incl. the per-call `llm-call` trace (`_trace_call`, `llm.py:471-490`). | Humans (`docs/deploy.md:159`). No code reads it. | 5 × 10 MiB rotation (`web_api.py:457`). | Nothing — a support trace, never evidence. |
...
| Replay artifact | caller-supplied path; JSON `{version, persona, ground_truth, final_state}` | `dump_replay_artifact` (`replay.py:158-169`); callers `scripts/dump_serde_goldens.py:99`, `tests/test_replay.py:143` |

Written once per run and never read by the product; reviewed or committed by hand.

### 1.1 Checkpoint shape (#1)

Tables from `langgraph-checkpoint-sqlite` 3.1.0 (`langgraph/checkpoint/sqlite/__init__.py:142-162`): `checkpoints(thread_id,
...
| `questions` | `{ts, kind, identity, questions}` — identity = `sha256(token)[:16]` or `"anonymous"` (`usage.py:668-680`) | `usage.py:309-318`, only via `reserve_questions` (`usage.py:900-905`) |
| `quota_exhausted` / `quota_retry` | `{ts, kind, provider}` — `provider` ∈ `ProviderName = groq\|openai\|zenmux` (`config.py:19`; `primary_provider` default `openai`, `:67`) | `usage.py:321-339` |
...
|---|---|---|---|---|
| `WebApiState.completed_sessions: dict[str, dict]` | `web_api.py:324` | `_remember_completed` (`web_api.py:1037-1044`) from `:949`, on any final state | export endpoint `web_api.py:689` | `MAX_COMPLETED_SESSIONS_IN_MEMORY = 64` (`:85`); oldest-first eviction `:1043-1044` under `lock`. The Markdown (#4) is written first (`:940-942`), so an evicted id is still exportable. |
| `WebApiState.runtimes: dict[str, RuntimeSession]` | `web_api.py:325` | registered by `_claim_session_id` (`web_api.py:1001-1034`: `:1006`, `:1030`); popped by whichever of socket-close (`:671-676`) / thread-exit (`:989-994`) happens last | duplicate-socket refusal `web_api.py:1008-1009`; `_is_running` `:790` | One entry per Session id. A closed socket whose thread is still inside a provider call keeps the entry until that thread exits. |
| `WebApiState.waiting: set[str]` | `web_api.py:327` | `add` `:1013`, `discard` `:1034` | `:1010` — a second waiter is refused (`_STILL_FINISHING`, `:998`) | One waiter per Session id; the join is capped at `STALE_RUNTIME_JOIN_SECONDS = 120` (`:82`, `:1018`). |
| `WebApiState.lock: threading.Lock` | `web_api.py:329` | — | guards every read/write of `runtimes`, `waiting` and the `completed_sessions` eviction (`:328`) | Process-wide. |
| `RuntimeSession` per socket: `answers`, `cancelled`, `thread`, `socket_closed`, `run_finished` | `web_api.py:281-291` | `answers.put_nowait` `:651`; `socket_closed = True` `:672`; `run_finished = True` `:990`; queue/event/flag reset per run `:293-299` | `QueueCandidate` `:234`; `run_in_flight` `:301-303` | `answers` is `queue.Queue(maxsize=ANSWER_QUEUE_MAXSIZE)` (= 8, `:77`, `:276-277`); overflow drops the answer with a `session_error` (`:652-656`). |
...
| `usage._RESERVATION_LOCK` | `usage.py:897` | — | `reserve_questions` `:902` serialises cap check + `questions` row | Process-wide; the reservation itself is a ledger row (#3). |
| `LLMRouter._breakers` | `llm.py:647` | `_record_success` `:664-672`, `_record_failure` `:674-691` | `breaker_is_open` `:651-662` | Process lifetime. |

**Single-process assumption.** All of these are process-local (the `ContextVar` narrower still). The server refuses
`--workers`/`WEB_CONCURRENCY` > 1 at import (`guard_single_worker`, `web_api.py:393-424`, called `:744`; `docs/deploy.md:101`).
A second process would miss reconnects, double-write the SQLite file, and run its own breakers, locks and fault latch.

**Import-time effects.** Importing `interview_coach.web_api` runs only `guard_single_worker()` and
`configure_session_logging(COACH_LOG_FILE)` (`web_api.py:744-745`). `web_api.app` is built lazily by the module
`__getattr__` on first attribute access (`:747-758`), so the import itself does not read `.env` (`load_settings`, `:530`),
open or sweep the checkpoint DB (`:703`), or mount static files (`:706`); those run when uvicorn or a test first touches `app`.
...
|---|---|---|
| 1 | A completed Session exists in the checkpoint (#1), RAM (bounded to 64, `web_api.py:949`) and Markdown (`web_api.py:942`); the export endpoint reads RAM then the file, never the checkpoint (`web_api.py:689-701`). | The checkpoint. RAM and the file are caches; the endpoint should render from #1 and fall back to the file only when the thread was pruned. |
...
| 3 | Provider retry constants restated in `usage.py:157-172` to avoid an import cycle (`usage.py:147-150`); pinned by `tests/test_usage.py:167`. | The owning modules (`evaluator`, `interviewer`, `llm`). `usage.py` is a derived worst-case model and keeps the pin test. |
...
(`web_api.py:745`, `COACH_LOG_FILE`) — while the other store paths go through it, and the CLI's argparse defaults for
