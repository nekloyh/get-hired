...
| 6 | ~~Chroma resource store (Study Planner)~~ | **Deleted, GH #130.** It was never on a default path — the web API hardcoded `memory`, both CLI call sites defaulted to it, no `Settings` field, no `"auto"` branch — and the catalog it indexed is two entries per Skill behind a hard `skill=` filter, so an embedder had nothing to rank. The Study Planner now always uses `InMemoryResourceStore` via `build_resource_store(seed=...)` (`resources.py`). `coach ingest-resources`, `--resource-store` and `--resource-persist-dir` are gone. | — | `search_resources` from `study_planner.py` | None | Nothing — an index over `SEED_RESOURCES` |
