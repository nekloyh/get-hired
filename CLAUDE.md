...
- **A missing middle BARS anchor is a language-fairness bug.** A dimension with a gap in its 1–5 scale lets the judge resolve the gap on *style*, and idiomatic Vietnamese reads as more authoritative than clumsy English — measured, not theorised: `correctness` had no `3` and split the same answer 2 (EN) vs 4 (VN). `depth` and `system_thinking` still have that hole. → `ADR 0009` addendum d
