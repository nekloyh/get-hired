...
- **Never swap the judge model silently — at merge time OR runtime.** Judge changes gate on `coach bench` (ADR 0009); the judge role is pinned and its failover never changes model (ADR 0009 addendum a). The bench is a stochastic measurement, so **the gate is median-of-k (k=3) at the production temperature** — one run is not a measurement, and a band is never widened to turn a red green (addenda b and d). **One green *invocation* is not a measurement either**: a judge change merges only on repeated invocations agreeing, and every re-wording restarts that count (addendum f — a #96 wording went 35/35, 35/35, 34/35). → `ADR 0009`, `ADR 0010`
- **A missing middle BARS anchor is a language-fairness bug** — and an *unscoped* anchor clause is the mirror image of one. A gap in a 1–5 scale lets the judge resolve it on *style*, and idiomatic Vietnamese reads as more authoritative than clumsy English (measured: `correctness` had no `3` and split the same answer 2 EN vs 4 VN). `depth` and `system_thinking` were anchored at 3 in GH #96; `communication` still has a gap at 2 (GH #103). Every anchor clause must name the boundary it decides and say it does not raise the bar above it — the first #96 wording read as a general rule and dragged strong answers from 4 to 3. → `ADR 0009` addenda d, e
...
**MiMo is dead** (endpoint retired 2026-06-03 — issue 0015 audit); any MiMo-primary instruction you find is stale. The validated judge is **OpenAI `gpt-5.4-mini`**, green at **35/35 under the median-of-k gate** (`docs/audits/calibration-bench-2026-07-27.md`, GH #92 closed — the 2026-07-19 AMBER flake is resolved, not carried). Groq is an *availability* fallback that does **not** pass the judge bench (18/20, VN Δ=2.00) — per ADR 0009's addendum the judge role must never fail over onto it. Per-role routing landed in #95, so the judge role is pinned in code; any judge-model switch is still a bench-gated change. All LLM calls go through the `LLMRouter` — no agent imports a provider client directly.
